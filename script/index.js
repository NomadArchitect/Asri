// const { default: fastdom } = require("fastdom");
...
(function () {
    ...
    let dragRectLeftInitial, dragRectRightInitial
    ...
    function calcTopbarSpacings(widthChange) {
        let pluginsDivider, leftSpacing, rightSpacing, topbar,
            layoutsCenterRect, leftSpacingRect, rightSpacingRect, barSyncRect, dragRect, barForwardRect,
            winWidth, centerRectLeft, centerRectRight;
    ...
    function statusPosition() {
        if (!isMobile) {
            if (!hasDockb()) {
                ...
                if (layoutCenter && asriDoms.layoutDockr) {
                    let layoutCenterWidth, layoutDockrWidth;

                    fastdom.measure(() => {
                        layoutDockrWidth = asriDoms.layoutDockr.offsetWidth;
                        layoutCenterWidth = layoutCenter.offsetWidth;
                    })

                    asriDoms.layoutDockb = asriDoms.layouts.querySelector('.layout__dockb');

                    if (asriDoms.layoutDockb && !asriDoms.layoutDockb.classList.contains('.fn__none') && isDockLytPinned(asriDoms.layoutDockb))
                        y = asriDoms.layoutDockb.offsetHeight * -1;
                    else y = 0;
            ...
            //     let layoutDockbHeight = asriDoms.layoutDockb?.offsetHeight;
    ...
    //     document.querySelector('#asriFastdom')?.remove;
    //     document.querySelector('#asriMainjs')?.remove;
